...
public class PortfolioTrackingScheduler
{
    ...
    private void processUser(User user, int dayCounter, boolean keepMe)
    {
        ...
        // "Budget aktualisieren: double delta = portfolioValue - previousPortfolioValue; newBudget =
        // currentBudget + delta;"
        ...
        double newCash = newBudget - currentPortfolioValue; // Should match previous cash unless budget logic
                                                            // drift?
        ...
        accountingRepository.savePerformance(user,
                                             newBudget,
    ...
    {
        List<User> users = userRepository.findAll();
        LocalDateTime threshold = LocalDateTime.now().with(LocalTime.MIDNIGHT); // "Aktueller Montag 00:00
                                                                                // Uhr"
