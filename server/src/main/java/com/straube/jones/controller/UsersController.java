...
public class UsersController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Preferences successfully saved.", 
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserPrefsResponse.class))),
            @ApiResponse(responseCode = "500", description = "Internal Server Error. Failed to save preferences due to file system or storage issues.",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserPrefsResponse.class)))
    })
    @PostMapping(path = "/prefs/{topic}", produces = "application/json")
    public UserPrefsResponse setUserPref(
            @Parameter(description = "The category or key for the preferences. Common values: 'filter', 'watchlist'.", required = true, example = "filter")
            @PathVariable String topic,
            
            @Parameter(description = "The JSON string representation of the preferences to store. This payload is stored blindly and returned as-is.", required = true, 
                    content = @Content(mediaType = "application/json", schema = @Schema(type = "string", example = "{\"minPrice\": 10, \"maxPrice\": 100}")))
            @RequestBody String userPrefs) {
        try {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Preferences successfully retrieved.", 
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserPrefsResponse.class))),
            @ApiResponse(responseCode = "500", description = "Internal Server Error. Failed to load preferences, possibly due to missing file or read permissions.",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserPrefsResponse.class)))
    })
    @GetMapping(path = "/prefs/{topic}", produces = "application/json")
    public UserPrefsResponse getUserPrefs(
            @Parameter(description = "The category or key of the preferences to retrieve.", required = true, example = "filter")
            @PathVariable String topic) {
        try {
            String preferences;
            if ("filter".equals(topic)) {
