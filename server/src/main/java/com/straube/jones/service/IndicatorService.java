...
public class IndicatorService {
     ...
     */
    public List<IndicatorDto> getIndicators(List<String> codes, Long startTime, Long endTime) {
        ...
        String sql = "SELECT cSymbol, cDateLong, cDayCounter, " +
        ...
        return namedParameterJdbcTemplate.query(sql, parameters, (rs, rowNum) -> {
            ...
            // Bollinger Bands
            dto.setBb15low(rs.getDouble("cBB15low"));
            if (rs.wasNull()) dto.setBb15low(null);
            
            dto.setBb15mid(rs.getDouble("cBB15mid"));
            if (rs.wasNull()) dto.setBb15mid(null);
            
            dto.setBb15high(rs.getDouble("cBB15high"));
            if (rs.wasNull()) dto.setBb15high(null);
            
            // RSI
            dto.setRsi(rs.getDouble("cRSI"));
            if (rs.wasNull()) dto.setRsi(null);
            
            // Volume
            dto.setVolume(rs.getDouble("cVolume"));
            if (rs.wasNull()) dto.setVolume(null);
            
            // MACD
            dto.setMacdValue(rs.getDouble("cMACDvalue"));
            if (rs.wasNull()) dto.setMacdValue(null);
            
            dto.setMacdSignal(rs.getDouble("cMACDsignal"));
            if (rs.wasNull()) dto.setMacdSignal(null);
            
            // RSI30 Probabilities
            dto.setRsi30Days5(rs.getDouble("cRSI30_5"));
            if (rs.wasNull()) dto.setRsi30Days5(null);

            dto.setRsi30Days10(rs.getDouble("cRSI30_10"));
            if (rs.wasNull()) dto.setRsi30Days10(null);
            
            dto.setRsi30Days20(rs.getDouble("cRSI30_20"));
            if (rs.wasNull()) dto.setRsi30Days20(null);
            
            dto.setRsi30Days30(rs.getDouble("cRSI30_30"));
            if (rs.wasNull()) dto.setRsi30Days30(null);
            
            dto.setRsi30probability(rs.getDouble("cRSI30probability"));
            if (rs.wasNull()) dto.setRsi30probability(null);
            
            // Simple Moving Averages
            dto.setSma5(rs.getDouble("cSMA5"));
            if (rs.wasNull()) dto.setSma5(null);
            
            dto.setSma10(rs.getDouble("cSMA10"));
            if (rs.wasNull()) dto.setSma10(null);
            
            dto.setSma20(rs.getDouble("cSMA20"));
            if (rs.wasNull()) dto.setSma20(null);
            
            dto.setSma30(rs.getDouble("cSMA30"));
            if (rs.wasNull()) dto.setSma30(null);
            
            // Exponential Moving Averages
            dto.setEma5(rs.getDouble("cEMA5"));
            if (rs.wasNull()) dto.setEma5(null);
            
            dto.setEma10(rs.getDouble("cEMA10"));
            if (rs.wasNull()) dto.setEma10(null);
            
            dto.setEma20(rs.getDouble("cEMA20"));
            if (rs.wasNull()) dto.setEma20(null);
            
            dto.setEma30(rs.getDouble("cEMA30"));
            if (rs.wasNull()) dto.setEma30(null);
            
            // Support and Resistance
            dto.setSupport(rs.getDouble("cSupport"));
            if (rs.wasNull()) dto.setSupport(null);
            
            dto.setResistance(rs.getDouble("cResistance"));
            if (rs.wasNull()) dto.setResistance(null);
    ...
    public void upsertIndicators(List<IndicatorDto> indicators) {
        ...
        String sql = "INSERT INTO tIndicators (" +
                     "cSymbol, cDateLong, cDayCounter, " +
                     "cBB15low, cBB15mid, cBB15high, " +
                     "cRSI, cVolume, " +
                     "cMACDvalue, cMACDsignal, " +
                     "cRSI30_5, cRSI30_10, cRSI30_20, cRSI30_30, cRSI30probability, " +
