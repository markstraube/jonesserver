...
public class AIAssistantService
{
    ...
    public AIAssistantService(ContextService contextService,
    ...
    public Flux<AIResponseChunk> explain(ExplainRequest request, String username)
    {
        ...
        return contextService.loadContext(sessionId, user, "explain").flatMapMany(context -> {
    ...
    public Flux<AIResponseChunk> analyze(AnalyzeRequest request, String username)
    {
        ...
        return Mono.fromCallable(() -> userRepository.findByUsername(username)).flatMap(userOpt -> {
            if (userOpt.isEmpty())
            ...
            LLMRequest llmRequest = LLMRequest.builder()
                                              ...
                                              .context(context.getMessages()
                                                              .subList(0, context.getMessages().size() - 1))
                                              .build();

            StringBuffer fullResponse = new StringBuffer();

            return llmService.streamResponse(llmRequest).doOnNext(chunk -> {
