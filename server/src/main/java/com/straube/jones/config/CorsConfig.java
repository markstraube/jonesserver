...
public class CorsConfig implements WebMvcConfigurer {
    ...
    public CorsConfig(@Value("${cors.allowed-origins:http://localhost:4200}") String originsProperty) {
        String[] parsedOrigins = Arrays.stream(originsProperty.split(","))
            .map(String::trim)
            .filter(origin -> !origin.isEmpty())
            .toArray(String[]::new);
        this.allowedOrigins = Objects.requireNonNull(parsedOrigins.length == 0 ? DEFAULT_ORIGINS : parsedOrigins);
    }
