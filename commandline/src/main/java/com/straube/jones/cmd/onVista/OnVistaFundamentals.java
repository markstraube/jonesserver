...
public class OnVistaFundamentals
{
    public static void main(String[] args)
        throws IOException
    {
        OnVistaFundamentals onVista = new OnVistaFundamentals("C:\\Dev\\__GIT\\jonesserver\\data\\onVista\\fundamentals");
        ...
        Path dirName = Path.of("C:\\Dev\\__GIT\\jonesserver\\data\\onVista\\finder2\\2022-12-01");
    ...
    public Long getStocksCount(String isin, String href)
    {
        String baseURL = String.format("https://www.onvista.de/aktien/unternehmensprofil/%s", href);
        try
        {
            ...
            for (int i = 0; i < sz; i++ )
            {
                ...
                Elements es = e.select(" div > div > dl > dt");
                if (es.isEmpty())
                {
                    continue;
                }
                String text = es.first().text();
                if (!"Anzahl Aktien".equals(text))
                {
                    continue;
                }
                Elements eData = e.select("div > div > dl > dd > data");                
                if (eData.isEmpty())
                {
                    continue;                    
                }
                String numStocks = eData.first().attr("value");
                return Long.parseLong(numStocks);
            }
        }
        catch (Exception ignore)
        {
